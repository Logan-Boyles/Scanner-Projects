...
def find_setup(history, tolerance=config.DEFAULT_SETUP_TOLERANCE, strict_tap=True):
    """
...
def generate_active_zones(day_history, window=config.DEFAULT_ZONE_WINDOW, enable_lookback=True, tolerance=config.DEFAULT_ZONE_TOLERANCE):
    """
    ...
    for i in range(0, max(0, len(day_history) - 2)):
        ...
        if abs(current['high'] - min_y_in_subset) <= tolerance:
            ...
            if abs(bottom - top) < config.MIN_ZONE_THICKNESS:
                bottom = top + config.MIN_ZONE_THICKNESS
        ...
        if abs(current['low'] - max_y_in_subset) <= tolerance:
            ...
            if abs(bottom - top) < config.MIN_ZONE_THICKNESS:
                top = bottom - config.MIN_ZONE_THICKNESS
...
def check_zone_taps(day_history, zones, lookback=config.DEFAULT_TAP_LOOKBACK, tap_tolerance=config.DEFAULT_TAP_TOLERANCE):
    """
...
def filter_new_session(history, vertical_gap_threshold=config.SESSION_GAP_THRESH):
    """
